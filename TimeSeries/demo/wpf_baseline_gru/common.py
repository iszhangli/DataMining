...
from demo.wpf_baseline_gru.model import BaselineGruModel
from demo.wpf_baseline_gru.wind_turbine_data import WindTurbineDataset
...
class Experiment(object):
    ...
    def get_data(self, flag):
        ...
        data_loader = DataLoader(
            ...
            shuffle=False,  # TODO shuffle_flag
            num_workers=self.args["num_workers"],
            drop_last=drop_last   # 最后一个不完整的数据集删除
        )
...
def traverse_wind_farm(method, params, model_path, flag='train'):
    ...
    for i in range(params["capacity"]):
        ...
        if 'train' == flag:
            method(exp, model_path, is_debug=params["is_debug"])   # train_and_val(experiment, model_folder, is_debug=False)
        elif 'test' == flag:
