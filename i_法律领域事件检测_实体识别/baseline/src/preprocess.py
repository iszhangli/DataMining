...
class Processor:
    ...
    def get_examples(self, raw_examples, set_type):
        ...
        for line in raw_examples.split('\n'):
            line = eval(line)
            labels = []
            if len(line['event_chain']) != 0:
                for tmp in line['event_chain']:
...
if __name__ == '__main__':
    ...
    label2id = {}
    id2label = {}
    with open('./law_data/final_data/labels.txt','r') as fp:
    ...
    train_out = get_out(processor, './law_data/final_data/train.json', args, label2id, 'train')
    dev_out = get_out(processor, './law_data/final_data/dev.json', args, label2id, 'dev')
