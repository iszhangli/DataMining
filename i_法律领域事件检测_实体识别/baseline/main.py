...
from src import bert_config, dataset, models, preprocess
import utils
import json
...
if __name__ == '__main__':
    ...
    log = os.path.join(args.log_dir, 'main.log')
    ...
    label2id = {}
    id2label = {}
    with open('./datasets/final_data/labels.txt', 'r') as fp:
    ...
    train_out = preprocess.get_out(processor, './datasets/final_data/train.json', args, label2id, 'train')
    ...
    dev_out = preprocess.get_out(processor, './datasets/final_data/dev.json', args, label2id, 'dev')
    ...
    # 读取test1.json里面的数据
    res = open("./results/result.json", 'w', encoding='utf-8')
    with open('./datasets/final_data/test1.json', 'r') as fp:
